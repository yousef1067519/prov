...
export async function GET(req: NextRequest) {
  ...
  // The plan drives the Meta Pixel Purchase value on the /welcome page.
  ...
  // Route through /welcome so the browser fires the Purchase pixel; it forwards
  // to the dashboard. If we couldn't confirm the plan, skip straight to the app.
  const dest = plan ? `/welcome?plan=${plan}` : '/dashboard?welcome=1'
