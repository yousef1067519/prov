...
// THE landing page — one design, many agencies. The generic homepage renders it
// with no vertical; /for/<slug> renders the exact same page with the copy swapped
// to that vertical (headline, subheadline, feature cards, FAQ wording). Design,
// robot hero, pipeline graph, stats, pricing, FAQ all stay identical.
...
function buildFaqs(v?: VerticalPreset): Array<{ q: string; a: string }> {
  // Swap the audience noun per vertical; everything else is product-true for all.
  const who = v ? v.label.toLowerCase() + 's' : 'influencer marketing agencies'
  const deal = v?.dealNoun ?? 'deal'
  return [
    {
      q: 'What does Prov actually do?',
      a: `Prov runs the ${deal} pipeline for ${who}: curated creator discovery, sponsor matching, team-approved outreach, contract generation, invoicing, FTC compliance tracking, and ${deal} tracking. Every completed ${deal} is written to a permanent intelligence record your agency can search.`,
    },
    {
      q: 'Who is it for?',
      a: `${v ? v.label + 's' : 'Established agencies'} and brand marketing teams — typically 5 to 50 people managing multiple client brands. If your process lives in spreadsheets, inboxes, and one senior employee’s head, Prov is built for exactly that.`,
    },
    {
    ...
    },
    {
      ...
      a: 'Self-serve tiers from $75/mo, plus Growth Agency and Enterprise (custom). You get the exact price in writing before you commit, and it never increases on you — that promise has been part of Prov from the start.',
    },
    {
      q: 'Who owns our data?',
      a: `You do. ${deal[0].toUpperCase() + deal.slice(1)} records, contracts, and intelligence belong to your agency; full export is available at any time, and nothing is retained after a contracted offboarding. We don’t sell your data or train models on your private records.`,
    },
    {
      q: 'Does Prov replace account managers?',
      a: `No — it multiplies them. Each account manager runs two to three times more ${deal}s because research, drafting, paperwork, and record-keeping are standardized and automated. Judgment stays human.`,
    },
  ]
}

export default function LandingPage({ vertical }: { vertical?: VerticalPreset }) {
  const v = vertical
  const faqs = buildFaqs(v)
  return (
    <>
      <LandingNav />
      <main style={{ background: 'var(--lp-bg)' }}>
        ...
        <section style={{ padding: '150px 24px 90px', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ maxWidth: 1120, margin: '0 auto', display: 'grid', gridTemplateColumns: 'minmax(0, 1.05fr) minmax(0, 1fr)', gap: 56, alignItems: 'center' }} className="lp-hero-grid">
            <div>
              <div className="lp-label" style={{ marginBottom: 20 }}>
                {v ? `▸ for ${v.label.toLowerCase()}s` : '▸ live pipeline'}
              </div>
              <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(2.1rem, 4.8vw, 3.4rem)', fontWeight: 700, color: '#fff', letterSpacing: '-0.03em', lineHeight: 1.08, marginBottom: 20 }}>
                {v ? v.headline : 'The fastest path from brief to booked creator.'}
              </h1>
              <p style={{ color: '#8a8a92', fontSize: '1.05rem', lineHeight: 1.65, maxWidth: 520, marginBottom: 30 }}>
                {v
                  ? v.subheadline
                  : 'Prov runs your sponsorship pipeline — research, outreach, contracts, compliance — and turns every completed deal into intelligence your agency owns. Not another inbox. An operating system for deals.'}
              </p>
              <HeroInput />
            </div>
            {/* Interactive 3D robot — the hero visual on every vertical */}
        ...
        <section id="pipeline" style={{ padding: '90px 24px', borderTop: '1px solid var(--lp-hairline)' }}>
          <div style={{ maxWidth: 1120, margin: '0 auto' }}>
            ...
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.5rem, 3.4vw, 2.2rem)', fontWeight: 700, color: '#fff', letterSpacing: '-0.02em', marginBottom: 10 }}>
              One brief in. One banked {v?.dealNoun ?? 'deal'} out.
            </h2>
        ...
        {/* ── Feature cards (vertical-specific text when provided) ── */}
        <FeatureRows items={v ? [...v.features, ...v.pains].map(x => ({ title: x.title, body: x.detail })).slice(0, 5) : undefined} />
